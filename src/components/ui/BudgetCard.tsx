  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center gap-2">
          {exceeded && (
            <span className="flex items-center gap-1 px-2 py-1 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-lg text-xs font-semibold">
              <i className="ri-alarm-warning-line text-xs"></i> Exceeded
            </span>
          )}
          {nearLimit && (
            <span className="flex items-center gap-1 px-2 py-1 bg-amber-50 dark:bg-amber-900/20 text-amber-500 rounded-lg text-xs font-semibold">
              <i className="ri-error-warning-line text-xs"></i> Near limit
            </span>
          )}
          <button
            onClick={onEdit}
      ...
      <div className="flex items-end justify-between mb-2">
        <span className="text-xl font-bold text-slate-800 dark:text-white">{formatCurrency(budget.spent)}</span>
        <span className="text-sm text-slate-400">of {formatCurrency(budget.amount)}</span>
      </div>
      <div className="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: barColor }} />
      </div>
      <p className="text-xs mt-2" style={{ color: barColor }}>{pct.toFixed(0)}% used</p>
    </div>
  );
}
