  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: t("reports.total_income"), value: fmt(totalIncome), icon: "ri-arrow-up-circle-line", color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/30" },
          { label: t("reports.total_expenses"), value: fmt(totalExpenses), icon: "ri-arrow-down-circle-line", color: "text-red-500", bg: "bg-red-50 dark:bg-red-900/30" },
          { label: t("reports.savings_rate"), value: `${savingsRate.toFixed(1)}%`, icon: "ri-safe-2-line", color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/30" },
        ].map((s) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-base font-bold text-slate-800 dark:text-white mb-1">{t("reports.by_category")}</h3>
          <p className="text-xs text-slate-400 mb-5">{t("reports.this_month")}</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-base font-bold text-slate-800 dark:text-white mb-1">{t("reports.breakdown")}</h3>
          <p className="text-xs text-slate-400 mb-5">{t("reports.top_categories")}</p>
          {categorySpend.length === 0 ? (
            <div className="h-40 flex items-center justify-center text-slate-400 text-sm">{t("reports.no_data")}</div>
          ) : (
